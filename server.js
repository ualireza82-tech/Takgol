...
async function translateWithGemini(text, targetLang = 'fa') {
  ...
  try {
    const prompt = targetLang === 'fa'
      ? `این جمله را به فارسی روان ترجمه کن (فقط ترجمه را بنویس و هیچ توضیح اضافه‌ای نده):\n${text}`
      : `Translate to English fluently (output only the translation):\n${text}`;

    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 8000); // ۸ ثانیه کات‌آف امن
    ...
    if (!response.ok) {
      ...
      console.error(`❌ Gemini API Error (${response.status}):`, errBody.substring(0, 150));
      return text;
    }
...
app.post('/api/translate', async (req, res) => {
  try {
    ...
    if (trimmedText.length < 3) {
      return res.json({ success: true, translated: text, cached: false, original: trimmedText });
    }
    ...
    res.json({ 
      ...
      translated, 
      cached: wasCached,
      original: trimmedText // 👈 بازگشت فیلد مورد نیاز فرانت‌اند
    });
