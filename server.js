// server.js — PostgreSQL (Neon) + SSE + persistent messages + self-ping + group + soft-delete (safe)
...
const RETENTION_DAYS = Number(process.env.RETENTION_DAYS || 180); // keep records for 180 days by default
...
function httpGetWithTimeout(urlStr, timeoutMs = SELF_PING_TIMEOUT_MS) {
  return new Promise((resolve, reject) => {
    ...
    const req = lib.request(url, options, (res) => { res.on("data", ()=>{}); res.on("end", ()=> resolve({ statusCode: res.statusCode })); });
...
// fetch messages filtered by group (excludes soft-deleted)
app.get("/messages", async (req, res) => {
  try {
    ...
    if (group) {
      q = `
        SELECT id, text, sender, phone, "group", reply, edited, created_at, ip, user_agent
        FROM messages
...
app.post("/send", async (req, res) => {
  try {
    ...
    const insert = `INSERT INTO messages (id, text, sender, phone, "group", reply, ip, user_agent) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING created_at`;
    const result = await pool.query(insert, [id, text, sender, phone, group, reply ? JSON.stringify(reply) : null, ip, ua]);

    // audit log (create)
    try {
      await pool.query(`INSERT INTO audit_logs (id,message_id,action,actor,meta) VALUES ($1,$2,'create',$3,$4)`, [uuidv4(), id, sender, JSON.stringify({ group, ip })]);
    } catch(e){ /* non-fatal */ }

    const msg = { id, text, sender, phone, group, reply, created_at: result.rows[0].created_at, ip, user_agent: ua };

    // broadcast includes group so clients can ignore non-matching groups
...
// soft-delete: mark deleted=true and keep record in DB (deleted_at + deleted_by)
// broadcast delete event so clients remove from UI, but DB retains record for RETENTION_DAYS
app.delete("/messages/:id", async (req, res) => {
  try {
    const id = req.params.id;
    const actor = req.headers['x-actor'] || req.body.actor || 'unknown';
    const updateQ = `UPDATE messages SET deleted = true, deleted_at = now(), deleted_by = $2 WHERE id = $1 RETURNING id, "group"`;
    const r = await pool.query(updateQ, [id, actor]);
    if (!r.rowCount) return res.status(404).json({ ok:false, error:"not found" });

    // audit
    try {
      await pool.query(`INSERT INTO audit_logs (id,message_id,action,actor,meta) VALUES ($1,$2,'soft-delete',$3,$4)`, [uuidv4(), id, actor, JSON.stringify({ reason: req.body.reason || null })]);
    } catch(e){}

    // notify clients: include id and group
    broadcastEvent({ type: "delete", id, group: r.rows[0]["group"] || null });
...
/* --------------- Cleanup (hard-delete only after retention on soft-deleted records) --------------- */
async function cleanupOldMessages() {
  try {
    // hard-delete messages that were soft-deleted and kept longer than retention window
    const q = `DELETE FROM messages WHERE deleted = true AND deleted_at < now() - ($1 || ' days')::interval`;
    const r = await pool.query(q, [RETENTION_DAYS]);
    if (r && r.rowCount) console.log(`cleanup: hard-deleted ${r.rowCount} messages older than ${RETENTION_DAYS} days (soft-deleted earlier)`);
  } catch (err) {
