/**
 * AJ Sports 2026 — RSS Bot Server v3.2 ⚡ CACHED EDITION + FREE TRANSLATE
 *
...
async function refreshLiveMatchesCache() {
  ...
  // اصلاح آدرس طبق مستندات نسخه ۳: حذف /api/ و استفاده از ساب‌دامین apiv3
  ...
  try {
    ...
    // مدیریت خطای احتمالی لایسنس یا ساختار خطای APIFootball
...
// این تابع متن را به صورت ۱۰۰٪ رایگان و بدون لیمیت ترجمه می‌کند.
// نیازی به هیچ API Key نیست. سرعت پردازش آن بالاست.
// ═══════════════════════════════════════════
async function translateWithFreeAPI(text, targetLang = 'fa') {
  ...
  try {
    ...
    // Endpoint مخفی و رایگان گوگل ترنسلیت برای درخواست‌های سرور (بدون کلید)
    ...
    // استخراج تکه‌های ترجمه‌شده از آرایه بازگشتی گوگل
    let translated = '';
    if (data && data[0]) {
      data[0].forEach(item => {
    ...
    // ذخیره در cache با مدیریت حافظه
