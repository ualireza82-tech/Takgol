// server.js — PostgreSQL (Neon) + SSE + persistent messages + self-ping + group + delete
  ...
  try {
    ...
    if (group) {
      q = `
        ...
        FROM messages
        WHERE "group" = $1 AND created_at >= now() - ($2 || ' days')::interval
...
// delete message by id (only deletes from DB and notifies clients)
