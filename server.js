...
app.post("/send", async (req, res) => {
  try {
    ...
    // 💡 اصلاح مویرگی ۱: مقدار پیش‌فرض را به 'Anonymous' (یا نامی که بهتر است) تغییر دهید یا بهتر است از NULL شروع کنید.
    let senderName = null; 
    ...
    if (email) {
      ...
      if (userResult.rowCount > 0) {
        ...
        // 💡 اصلاح مویرگی ۲: تضمین می‌کند که اگر نام و نام خانوادگی خالی بود، حتماً از ایمیل استفاده شود.
        const fullName = `${user.first_name || ''} ${user.last_name || ''}`.trim();
        senderName = fullName || email; // اگر نام کامل خالی بود، ایمیل را به عنوان نام ذخیره کن
        
        userAvatarBase64 = user.avatar_base64 || null;
      } else {
        // 💡 اصلاح مویرگی ۳: اگر ایمیل بود اما کاربر پیدا نشد، حداقل ایمیل را به عنوان نام ثبت کن.
        senderName = email; 
      }
    }

    // 💡 اصلاح مویرگی ۴: اگر email هم خالی بود، اکنون پیش فرض 'کاربر' را قرار دهید.
    if (!senderName) {
        senderName = "کاربر";
    }
    ...
    `;
    // 💡 نکته: senderName اکنون تضمین شده است که یا نام کامل، یا ایمیل، یا نهایتاً 'کاربر' باشد.
