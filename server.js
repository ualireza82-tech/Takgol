// server.js — SSE compatible + Neon (Postgres) persistent messages + soft-delete + self-ping
...
/* --------------- CONFIG --------------- */
// DATABASE_URL باید در ENV تنظیم باشه
...
const SELF_PING_JITTER_MS = Number(process.env.SELF_PING_JITTER_MS || 30000);
const SELF_PING_TIMEOUT_MS = Number(process.env.SELF_PING_TIMEOUT_MS || 10000);
const SSE_KEEPALIVE_INTERVAL_MS = Number(process.env.SSE_KEEPALIVE_INTERVAL_MS || 20000);
...
let clients = []; // array of response objects

function broadcastEvent(obj) {
  const payload = `data: ${JSON.stringify(obj)}\n\n`;
  // best-effort broadcast; remove closed clients lazily
...
// GET /messages?group=...&limit=...
// returns only not-deleted messages by default (deleted = false)
...
// POST /send  — store message (keeps record), broadcast {type:'message', payload:msg}
app.post("/send", async (req, res) => {
  try {
    ...
    // audit log create (best-effort)
    try {
      await pool.query(`INSERT INTO audit_logs (id,message_id,action,actor,meta) VALUES ($1,$2,'create',$3,$4)`, [uuid(), id, sender, JSON.stringify({ group, ip })]);
    } catch (e) { /* non-fatal */ }

    const msg = { id, text, sender, phone, group, reply, created_at: result.rows[0].created_at, ip, user_agent: ua };

    // broadcast message — clients should check group and ignore if not relevant
...
// SOFT DELETE: mark deleted=true, keep DB record; broadcast delete event so UI removes
app.delete("/messages/:id", async (req, res) => {
  try {
    ...
    try {
      await pool.query(`INSERT INTO audit_logs (id,message_id,action,actor,meta) VALUES ($1,$2,'soft-delete',$3,$4)`, [uuid(), id, actor, JSON.stringify({ reason: req.body.reason || null })]);
    } catch (e) { /* non-fatal */ }

    const group = r.rows[0]["group"] || null;
    // broadcast delete event (include group so clients can decide)
    broadcastEvent({ type: "delete", id, group });
...
/* --------------- Cleanup (hard-delete after retention window) --------------- */
async function cleanupOldMessages() {
  try {
    ...
    if (r && r.rowCount) console.log(`cleanup: hard-deleted ${r.rowCount} old soft-deleted messages`);
  } catch (err) {
