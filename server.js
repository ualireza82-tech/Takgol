// server.js — PostgreSQL (Neon) + SSE + persistent messages + self-ping + group + soft delete + image support
...
const app = express();
app.use(cors());
app.use(express.json({ limit: '10mb' })); // افزایش محدودیت برای عکس Base64
...
/* --------------- SCHEMA (include group, soft delete, image) --------------- */
...
app.get("/messages", async (req, res) => {
  try {
    ...
    if (group) {
      q = `
        SELECT id, text, sender, phone, "group", created_at, ip, user_agent, image
        FROM messages
...
app.post("/send", async (req, res) => {
  try {
    ...
    const image = req.body.image ? String(req.body.image) : null; // Base64
    if (!text && !image) return res.status(400).json({ ok:false, error:"empty text and image" });
    ...
    `;
    const result = await pool.query(insert, [id, text || "", sender, phone, group, ip, ua, image]);

    const msg = { id, text, sender, phone, group, created_at: result.rows[0].created_at, ip, user_agent: ua, image };
