...
// 🎯 نقطه زنی: Endpoint جدید برای ثبت نام یا ورود
...
app.post("/send", async (req, res) => {
  try {
    ...
    // 1. واکشی اطلاعات فرستنده از دیتابیس
    let senderName = "کاربر";
    let messageImageBase64 = req.body.image ? String(req.body.image) : null; // عکس ارسالی در پیام
    ...
    if (phone) {
      ...
      if (userResult.rowCount > 0) {
        ...
        senderName = fullName || "کاربر";
        // ⚠️ اگر در پیام، عکسی ارسال نشده باشد، می‌توان از آواتار پروفایل استفاده کرد:
        // اگر messageImageBase64 خالی است، آن را با آواتار پروفایل پر کن.
    ...
    const msg = { id, text, sender: senderName, phone, group, created_at: result.rows[0].created_at, ip, user_agent: ua, image: messageImageBase64 };
