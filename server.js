...
const ensureSchema = async () => {
  const create = `
    ...
    CREATE TABLE IF NOT EXISTS users (
      ...
      last_name TEXT,
      email TEXT,         -- <--- اضافه شده
      avatar_base64 TEXT,
      avatar_url TEXT,    -- <--- اضافه شده
    ...
    -- <--- اضافه شده: اطمینان از وجود ستون‌های جدید users
    DO $$ BEGIN ALTER TABLE users ADD COLUMN IF NOT EXISTS email TEXT; EXCEPTION WHEN others THEN END $$;
    DO $$ BEGIN ALTER TABLE users ADD COLUMN IF NOT EXISTS avatar_url TEXT; EXCEPTION WHEN others THEN END $$;
  `;
  ...
  try {
    ...
    const baseQuery = `
        SELECT 
            ...
            COALESCE(u.avatar_url, u.avatar_base64) AS avatar, -- <--- اصلاح شده
            m.created_at
        FROM messages m
    ...
    const rows = r.rows.map(row => ({
        ...
        avatar: row.avatar || null, // <--- avatar اکنون URL یا Base64 است
...
app.post("/send", async (req, res) => {
  try {
    ...
    let userAvatarData = null; // <--- اصلاح شده

    if (phone) {
      // <--- اصلاح شده: avatar_url و avatar_base64 را بخوانید
      const userResult = await pool.query('SELECT first_name, last_name, avatar_base64, avatar_url FROM users WHERE phone = $1', [phone]);
      if (userResult.rowCount > 0) {
        const user = userResult.rows[0];
        senderName = `${user.first_name || ''} ${user.last_name || ''}`.trim() || "کاربر";
        // <--- اصلاح شده: اگر URL وجود داشت، آن را استفاده کنید، در غیر این صورت Base64
        userAvatarData = user.avatar_url || user.avatar_base64 || null; 
      }
    }
    ...
    const msg = { 
      ...
      text, 
      sender: senderName, 
      sender_phone_id: phone, 
      avatar: userAvatarData || null, // <--- ارسال URL یا Base64 عکس
