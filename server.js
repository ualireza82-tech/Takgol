// server.js
// PostgreSQL (Neon) + SSE + persistent messages + soft-delete + edit + audit + self-ping
...
// basic process exit on unhandled rejection to avoid silent failures in free envs
process.on('unhandledRejection', (err) => {
  console.error('unhandledRejection', err);
  // do not exit in production; just log
});
...
async function ensureSchema() {
  const create = `
    ...
    CREATE TABLE IF NOT EXISTS audit_logs (
      id TEXT PRIMARY KEY,
      message_id TEXT,
      action TEXT NOT NULL,    -- 'delete' | 'restore' | 'edit' | 'create' | 'view_deleted'
      actor TEXT,
      meta JSONB,
      created_at TIMESTAMPTZ DEFAULT now()
    );
...
function broadcast(obj){
  ...
  clients.forEach(c => {
    try { c.write(payload); } catch (e) { /* ignore write error - cleanup on close */ }
  });
}
...
// GET messages (default: exclude deleted). Query: ?group=...&include_deleted=true
...
// SSE endpoint (clients subscribe)
app.get("/events", (req, res) => {
  res.writeHead(200, { "Content-Type":"text/event-stream", "Cache-Control":"no-cache", Connection:"keep-alive" });
  // initial comment to keep proxies happy
  res.write(":\n\n");
  clients.push(res);
  req.on("close", () => { clients = clients.filter(c => c !== res); });
});

// POST /send  (store message + broadcast)
// expected body: { text, sender, group, phone, reply }
app.post("/send", async (req, res) => {
  try {
    ...
    const reply = req.body.reply ? req.body.reply : null; // keep as object
    const id = uuidv4();
...
// PUT /messages/:id  -> edit message (soft edit, keep audit)
app.put("/messages/:id", async (req, res) => {
  try {
    ...
    // audit
    const auditId = uuidv4();
    await pool.query(`INSERT INTO audit_logs (id,message_id,action,actor,meta) VALUES ($1,$2,'edit',$3,$4)`, [auditId, id, editor, JSON.stringify({ newText })]);
...
// DELETE /messages/:id -> SOFT delete (deleted=true), keep all data in DB, audit log, broadcast hide event
app.delete("/messages/:id", async (req, res) => {
  try {
    ...
    // audit log
    const auditId = uuidv4();
    await pool.query(`INSERT INTO audit_logs (id,message_id,action,actor,meta) VALUES ($1,$2,'delete',$3,$4)`, [auditId, id, actor, JSON.stringify({ reason: req.body.reason || null })]);

    // broadcast delete/hide event
...
/* --------------- Background cleanup (hard delete older than retention) --------------- */
async function cleanupOldMessages(){
  try {
    // Hard delete only messages older than retention AND already marked deleted=true
    ...
    if (r && r.rowCount) console.log(`cleanup: hard-deleted ${r.rowCount} old messages (previously soft-deleted)`);
  } catch (err) {
