// server.js — PostgreSQL (Neon) + SSE + persistent messages + self-ping + group + soft delete
...
// اگر می‌خواهید پیام‌های حذف شده را هم بعد از مدتی حذف کنید، می‌توانید از یک RETENTION_DAYS_SOFT_DELETE جداگانه استفاده کنید.
...
app.get("/messages", async (req, res) => {
  try {
    ...
    // 💡 تغییر کلیدی: فیلتر کردن پیام‌های حذف شده (deleted = false)
    const baseQuery = `
...
app.post("/send", async (req, res) => {
  try {
    ...
    // توجه: پیام‌های جدید با deleted=false ذخیره می‌شوند (که مقدار پیش‌فرض است)
...
app.delete("/messages/:id", async (req, res) => {
  try {
    ...
    // 💡 تغییر کلیدی: UPDATE به‌جای DELETE
    // فقط فلگ حذف رو می‌زنیم، از دیتابیس حذف نمی‌کنیم
    const q = `UPDATE messages SET text = '[پیام حذف‌شده]', deleted = true, deleted_at = now() WHERE id = $1`;
    const r = await pool.query(q, [id]);
    if (r.rowCount) {
      broadcastEvent({ type: "delete", id }); // اطلاع‌رسانی به کلاینت‌ها برای حذف ظاهری
      res.json({ ok: true });
    } else {
...
async function cleanupOldMessages() {
  try {
    // 💡 تغییر کلیدی: فقط پیام‌های "حذف‌نشده" قدیمی‌تر از RETENTION_DAYS را حذف فیزیکی می‌کند.
    // پیام‌های حذف‌شده (deleted=true) برای همیشه (یا تا زمانی که بخواهید) باقی می‌مانند.
